...
function DonutChart({ summaries }: { summaries: Summary[] }) {
  ...
  const bands = [
    { label: "Affordable",    color: "#22c55e", count: summaries.filter(s => s.avg < 6).length },
    { label: "Moderate",      color: "#eab308", count: summaries.filter(s => s.avg >= 6 && s.avg < 8).length },
    { label: "Expensive",     color: "#f97316", count: summaries.filter(s => s.avg >= 8 && s.avg < 10).length },
    { label: "Very Expensive",color: "#ef4444", count: summaries.filter(s => s.avg >= 10).length },
  ].filter(b => b.count > 0);
  ...
  const slices = bands.map(b => {
    const angle = (b.count / total) * 2 * Math.PI * progress;
    ...
    cumAngle += angle;
    return { ...b, d, midAngle, pct: Math.round((b.count / total) * 100) };
  });
  ...
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 20 }}>
      <svg viewBox="0 0 160 160" style={{ width: 160, height: 160, flexShrink: 0 }}>
        <defs>
          {bands.map(b => (
            <filter key={b.color} id={`glow${b.color.slice(1)}`}>
        ...
        {slices.map((s, i) => (
          <path key={i} d={s.d} fill={s.color}
            style={{
              opacity: tip && tip !== s.label ? 0.4 : 1,
              filter: tip === s.label ? `drop-shadow(0 0 6px ${s.color})` : undefined,
        ...
        {/* centre hole label */}
        <circle cx={cx} cy={cy} r={r - 2} fill="rgba(10,22,40,0.92)" />
        <text x={cx} y={cy - 8} textAnchor="middle" style={{ fill: "white", fontSize: 18, fontWeight: 700 }}>
          {total}
        </text>
        <text x={cx} y={cy + 8} textAnchor="middle" style={{ fill: "rgba(255,255,255,0.45)", fontSize: 8 }}>
          DISTRICTS
        </text>
        {tip && (() => {
          const s = slices.find(s => s.label === tip);
          return s ? (
            <>
              <text x={cx} y={cy + 20} textAnchor="middle" style={{ fill: s.color, fontSize: 7, fontWeight: 600 }}>{s.label}</text>
              <text x={cx} y={cy + 30} textAnchor="middle" style={{ fill: "white", fontSize: 9, fontWeight: 700 }}>{s.pct}%</text>
            </>
          ) : null;
        })()}
      </svg>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 8, flex: 1 }}>
        {slices.map(s => (
          <div key={s.label}
            style={{ display: "flex", alignItems: "center", gap: 8, cursor: "pointer", opacity: tip && tip !== s.label ? 0.4 : 1, transition: "opacity .2s" }}
            onMouseEnter={() => setTip(s.label)}
            onMouseLeave={() => setTip(null)}>
            <span style={{ width: 10, height: 10, borderRadius: "50%", background: s.color, flexShrink: 0, boxShadow: `0 0 6px ${s.color}88` }} />
            <span style={{ fontSize: 11, color: "rgba(255,255,255,0.75)", flex: 1 }}>{s.label}</span>
            <span style={{ fontSize: 12, fontWeight: 700, color: s.color, tabularNums: true } as React.CSSProperties}>
              {s.count} <span style={{ fontWeight: 400, fontSize: 10, color: "rgba(255,255,255,0.4)" }}>({s.pct}%)</span>
            </span>
          </div>
        ))}
      </div>
...
export default function HousingPage() {
  ...
  const kpis = [
    ...
    { icon:"✅", val: loading?"—":`${affordable}`,               label:"Affordable",     sub:"below €6/m²",                             color:"#4ade80" },
      ...
      <div className="px-4 lg:px-8 py-8">
        ...
        {!loading && !error && summaries.length > 0 && (
          <div style={{ display:"flex",flexDirection:"column",gap:24 }}>
            ...
            {/* ── NEW: Donut + compact top-10 ranked list */}
            <div className="card-glass p-5 fade-up" style={{ animationDelay:"0.05s" }}>
              <div className="flex items-center justify-between mb-4">
                <div>
                  <h2 className="text-lg font-bold text-white">🏠 Avg Net Cold Rent — All Districts</h2>
                  <p className="text-[10px] mt-0.5" style={{ color:"rgba(255,255,255,0.35)" }}>Mietspiegel Magdeburg 2024 · hover slices &amp; rows</p>
                </div>
              </div>
              <div style={{ display:"grid",gridTemplateColumns:"auto 1fr",gap:24,alignItems:"start" }}>
                ...
                {/* compact ranked list — top 10 by avg */}
                <div>
                  <div className="text-[10px] font-semibold uppercase tracking-widest mb-2" style={{ color:"rgba(255,255,255,0.35)" }}>Top 10 by avg rent</div>
                  <div style={{ display:"flex",flexDirection:"column",gap:5 }}>
                    {[...summaries].sort((a,b)=>b.avg-a.avg).slice(0,10).map((s,i)=>{
                      const rc  = rentColor(s.avg);
                      const pct = (s.avg/maxAvg)*100;
                      return (
                        <div key={s.stadtteil} style={{ display:"flex",alignItems:"center",gap:8 }}>
                          <span style={{ width:16,fontSize:9,color:"rgba(255,255,255,0.3)",textAlign:"right",flexShrink:0 }}>{i+1}</span>
                          <span style={{ fontSize:11,color:"rgba(255,255,255,0.75)",width:120,flexShrink:0,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis" }}>{s.stadtteil}</span>
                          <div style={{ flex:1,height:6,background:"rgba(255,255,255,0.06)",borderRadius:3,overflow:"hidden" }}>
                            <div className="bar-grow" style={{ height:"100%",borderRadius:3,width:`${pct}%`,background:`linear-gradient(to right,${rc.hex}88,${rc.hex})`,animationDelay:`${i*30}ms` }} />
                          </div>
                          <span style={{ fontSize:11,fontWeight:700,color:rc.hex,minWidth:44,textAlign:"right" }}>€{s.avg.toFixed(2)}</span>
                        </div>
                      );
                    })}
                  </div>
                ...
                {[...summaries].sort((a,b)=>a.avg-b.avg).slice(0,8).map((s,i)=>{
                  ...
                  return (
                    <div key={s.stadtteil}
                      ...
                      style={{ background:"rgba(255,255,255,0.04)",border:`1px solid ${rc.hex}33`,animation:"fadeUp .5s ease both",animationDelay:`${0.2+i*0.06}s`,transition:"transform .2s,box-shadow .2s" }}
                        ...
                        <tr key={s.stadtteil}
                          ...
                          <td className="px-5 py-2.5">
                            <div style={{ position:"relative",height:6,width:80,background:"rgba(255,255,255,0.06)",borderRadius:3 }}>
                              <div style={{ position:"absolute",height:"100%",borderRadius:3,left:`${Math.max(0,((s.min-minAll)/(maxAvg-minAll))*80)}px`,width:`${Math.max(4,(span/maxAvg)*80)}px`,background:`linear-gradient(to right,${rc.hex}88,${rc.hex})` }} />
                            </div>
                          </td>
                        </tr>
                      );
